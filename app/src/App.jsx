...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!cancelled) await DS.saveState(st, sp);
      } catch (e) {
        console.error(e);
      }
    })();
  ...
  const onCreate = async (pass, confirm) => {
    setAuthErr("");
    if (pass.length < 8) {
      setAuthErr("Min. 8 caractères.");
      return;
    }
    if (pass !== confirm) {
      setAuthErr("Différents.");
      return;
    }
  ...
  const currentTab = ["home", "todos", "budget", "calendar", "notes"].includes(st?.activeTab)
    ? st.activeTab
    : "home";
  ...
  return (
    ...
    >
      ...
      >
        <TopBtn onClick={() => setShowChat(true)}>
          <Icons.MessageCircle size={18} color={C.accent} />
        </TopBtn>
        <TopBtn onClick={() => setShowSettings(true)}>
          <Icons.Settings size={18} color={C.text2} />
        </TopBtn>
        <TopBtn onClick={onLock}>
          <Icons.Lock size={18} color={C.text2} />
        </TopBtn>
      </div>

      {currentTab === "home" && <HomeModule st={st} go={(t) => uf("activeTab", t)} />}
      {currentTab === "todos" && (
        <TodoModule
          todos={st.todos}
          setTodos={(n) => uf("todos", n)}
          events={st.events}
          setEvents={(n) => uf("events", n)}
        />
      )}
      {currentTab === "budget" && (
        <BudgetModule
          transactions={st.transactions}
          setTransactions={(n) => uf("transactions", n)}
          monthlyBudget={st.monthlyBudget}
          setMonthlyBudget={(n) => uf("monthlyBudget", n)}
          investments={st.investments}
          setInvestments={(n) => uf("investments", n)}
        />
      )}
      {currentTab === "calendar" && (
        <CalendarModule
...
function UnlockScreen({ mode, error, busy, onCreate, onUnlock, hasExisting }) {
  ...
  useEffect(() => {
    setTimeout(() => r.current?.focus(), 120);
  }, []);
      ...
      <div style={{ width: "100%", maxWidth: 390 }}>
        <Card style={{ padding: 20 }}>
          ...
          {isCreate ? (
            <SubmitBtn
              disabled={busy || !p || !c}
              onClick={() => onCreate(p, c)}
              text={busy ? "Création..." : "Créer et ouvrir"}
            />
          ) : (
            <SubmitBtn
              disabled={busy || !p}
              onClick={() => onUnlock(p)}
              text={busy ? "Déchiffrement..." : "Déverrouiller"}
            />
          )}
