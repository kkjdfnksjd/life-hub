...
export function CalendarModule({ events, setEvents }) {
  ...
  return (<div>
    <div style={{ padding: "16px 20px 0" }}><h1 style={{ fontSize: 34, fontWeight: 700, letterSpacing: -0.5, margin: 0 }}>Calendrier</h1><ProgressBar value={wEv.filter((e) => e.date < todayKey).length} max={wEv.length} label={wEv.length + " évén. cette sem."} color={C.accent} /></div>
    ...
    <div style={{ padding: "4px 20px 12px" }}>{se.length === 0 ? <Card><Empty text="Rien de prévu" /></Card> : <Card>{se.map((ev, i) => <EvRow key={ev.id} ev={ev} last={i === se.length - 1} onDel={() => setEvents((p) => p.filter((e) => e.id !== ev.id))} />)}</Card>}</div>
    <div style={{ padding: "8px 20px 4px" }}><span style={{ fontSize: 13, fontWeight: 600, color: C.text2, textTransform: "uppercase", letterSpacing: .5 }}>À venir (30j)</span></div>
