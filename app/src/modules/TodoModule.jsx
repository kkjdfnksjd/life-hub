...
export function TodoModule({ todos, setTodos, events, setEvents }) {
  ...
  const toggleTodo = (todo) => {
    ...
    const completedAt = nextDone ? new Date().toISOString() : "";

    setTodos((p) =>
      p.map((t) =>
        t.id === todo.id
          ? { ...t, done: nextDone, completedAt }
          : t
      )
    );
  ...
  const deleteTodo = (todo) => {
    ...
    if (todo.linkedEventId) {
      setEvents((p) => p.filter((e) => e.id !== todo.linkedEventId));
    }
    setSwId(null);
  };

  const clearDone = () => {
    const doneLinkedIds = todos
      .filter((t) => t.done && t.linkedEventId)
      .map((t) => t.linkedEventId);

    setTodos((p) => p.filter((t) => !t.done));

    if (doneLinkedIds.length > 0) {
      setEvents((p) => p.filter((e) => !doneLinkedIds.includes(e.id)));
    }
  };
    ...
    <div>
      <div style={{ padding: "16px 20px 0" }}>
        ...
        <ProgressBar
          value={dc}
          max={tot}
          label={ac ? dc + "/" + tot + " " + ac.label.toLowerCase() : dc + "/" + tot + " terminées"}
          color={ac ? ac.color : C.green}
        />
      </div>
          ...
          {sorted.length === 0 ? (
            ...
            sorted.map((todo, i) => (
              <TodoItem
                ...
                isSwiped={swId === todo.id}
                onSwipe={() => setSwId(swId === todo.id ? null : todo.id)}
              />
            ))
          )}
        </Card>
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          padding: "14px 16px",
          background: C.card,
          borderBottom: isLast ? "none" : ".5px solid " + C.separator,
      ...
      >
        <div
          style={{
            ...
            height: 24,
            borderRadius: 12,
            border: todo.done ? "none" : "2px solid " + C.text3,
            background: todo.done ? C.green : "transparent",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            flexShrink: 0,
        ...
        <div style={{ flex: 1 }}>
          ...
          {todo.dueDate && (
            <div
              style={{
                fontSize: 11,
                marginTop: 3,
                color: over ? C.red : isToday ? C.orange : C.text3,
            ...
            >
              <Icons.Calendar size={11} color={over ? C.red : isToday ? C.orange : C.text3} />
              {over ? "En retard — " : isToday ? "Aujourd'hui — " : ""}
          ...
          <LocationLink location={todo.location} />
        </div>
    ...
    <>
      ...
      <ModalSheet>
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && text.trim()) onAdd(text.trim(), cat, due, location);
          }}
          style={inputStyle}
          onFocus={focusBorder}
          onBlur={blurBorder}
        ...
        <div style={{ marginTop: 10 }}>
          <label style={{ fontSize: 13, color: C.text2, fontWeight: 500 }}>Échéance (optionnel)</label>
          <input
            type="date"
            value={due}
            onChange={(e) => setDue(e.target.value)}
            style={{ ...inputStyle, marginTop: 6, padding: "12px 16px", fontSize: 14 }}
          />
        </div>
