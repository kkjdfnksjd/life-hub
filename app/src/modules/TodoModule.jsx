...
export function TodoModule({ todos, setTodos, events, setEvents }) {
  ...
  return (<div>
    <div style={{ padding: "16px 20px 0" }}><h1 style={{ fontSize: 34, fontWeight: 700, letterSpacing: -0.5, margin: 0 }}>Mes tâches</h1><ProgressBar value={dc} max={tot} label={ac ? dc + "/" + tot + " " + ac.label.toLowerCase() : dc + "/" + tot + " terminées"} color={ac ? ac.color : C.green} /></div>
...
function AddTodoModal({ onAdd, onClose }) {
  ...
  return (<><Backdrop onClick={onClose} /><ModalSheet>
    ...
    <div style={{ marginTop: 10 }}><label style={{ fontSize: 13, color: C.text2, fontWeight: 500 }}>Échéance (optionnel)</label><input type="date" value={due} onChange={(e) => setDue(e.target.value)} style={{ ...inputStyle, marginTop: 6, padding: "12px 16px", fontSize: 14 }} /></div>
