...
export function FAB({ onClick }) {
  return (<button onClick={onClick} style={{ position: "fixed", bottom: "calc(110px + env(safe-area-inset-bottom))", right: "max(20px, calc(50% - 195px))", width: 56, height: 56, borderRadius: 28, background: C.accent, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 4px 16px rgba(0,122,255,.35)", zIndex: 50 }} onMouseDown={(e) => (e.currentTarget.style.transform = "scale(.92)")} onMouseUp={(e) => (e.currentTarget.style.transform = "scale(1)")} onMouseLeave={(e) => (e.currentTarget.style.transform = "scale(1)")}><Icons.Plus size={28} color="white" /></button>);
}
